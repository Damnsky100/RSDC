...
def generate_p(params,y) :
    ...
    for t in range(1, len(y)) : 
        ...
        fyt_global[t] = 1/(math.sqrt(2*np.pi*sig))*math.exp(-1*(y[t]-u0)**2/(2*sig))*p1_prev[t] + 1/(math.sqrt(2*np.pi*sig2))*math.exp(-1*(y[t]-u1)**2/(2*sig2))*p2_prev[t]
    
        p1filt[t] = (1/(math.sqrt(2*np.pi*sig)))*math.exp(-1*(y[t]-u0)**2/(2*sig))* p1_prev[t] / fyt_global[t]
        p2filt[t] = (1/(math.sqrt(2*np.pi*sig2)))*math.exp(-1*(y[t]-u1)**2/(2*sig2))* p2_prev[t] / fyt_global[t]
...
def hamilton(params,y):
    ...
    for t in range(1,len(y)):
        ...
        #etape 2
        f[t] = 1/(math.sqrt(2*np.pi*sig))*math.exp(-1*(y[t]-u0)**2/(2*sig))*p1pred + 1/(math.sqrt(2*np.pi*sig2))*math.exp(-1*(y[t]-u1)**2/(2*sig2))*p2pred
...
def optimize(y):
    ...
    bounds = ((0,None),(0,None),(0.1,2),(0.001,1),(0.001,1),(0.1,2))
    xo=[0.05,0.01,1,0.7,0.7,1]
...
def MonteCarloSimulate(count):
    ...
    for i in tqdm(range(count)):
        ...
        sig2 = optimizeroutput.x[5]

        df.loc[len(df)]=[u0,u1,sig,p11,p22,sig2]
